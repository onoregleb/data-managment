...
def extract_from_mongodb(**context):
    ...
    sync_state.update_one(
        {"_id": "last_sync"},
        {"$set": {"timestamp": current_sync_time}},
        upsert=True
    )
    
    client.close()
    
    print(f"📤 Extracted NEW data from MongoDB: {len(wallets)} wallets, {len(transactions)} transactions")
...
def load_to_postgres(**context):
    ...
    postgres_uri = os.getenv('POSTGRES_URI', 'postgresql://postgres:postgres@postgres-dw:5432/blockchain')
...
def print_statistics(**context):
    ...
    if load_result:
        print(f"📥 Loaded this run:      {load_result.get('transactions_loaded', 0)} tx, {load_result.get('wallets_loaded', 0)} wallets")
